	...
	@if (Session::has('error'))
		...
		<div><strong>{{ Session::get('error') }}
				@if (Session::has('verify-email'))
					...
					{{--<form action="{{url('send-verify')}}" method="post" style="display: block; margin: 0">--}}
						{{--@csrf--}}
						{{--<input type="hidden" value="{!! Session::get('verify-email') !!}}" name="vemail">--}}
						...
						{{--<button type="submit" style="background-color:transparent; border-color:transparent; display: inherit; margin: 0">--}}
							{{--<a href="" class="">yeni verifikasiya göndər</a>--}}
						{{--</button>--}}
					{{--</form>--}}
				@endif
			</strong>
